...
import com.insta360.kmpsdk.demo.util.PcDetectBridge
...
class PreviewFragment : Fragment() {
    ...
    private val pcDetectBridge = PcDetectBridge()
    ...
    override fun onViewCreated(
        ...
        binding.pcDetectBtn.setOnClickListener { togglePcDetect() }
    ...
    private fun togglePcDetect() {
        if (pcDetectBridge.running) {
            stopPcDetect()
            return
        }
        val player = previewPlayerView
        if (player == null || !previewStarted || player.width <= 2) {
            Toast.makeText(requireContext(), R.string.pc_detect_need_preview, Toast.LENGTH_SHORT).show()
            return
        }
        val activity = requireActivity()
        pcDetectBridge.start(
            scope = viewLifecycleOwner.lifecycleScope,
            activity = activity,
            previewView = { previewPlayerView },
        ) { status ->
            activity.runOnUiThread {
                if (_binding == null) return@runOnUiThread
                when (status) {
                    PcDetectBridge.Status.Sending -> {
                        binding.pcDetectBtn.text = getString(R.string.pc_detect_stop)
                        binding.pcDetectStatus.text = getString(R.string.pc_detect_status_sending)
                    }
                    PcDetectBridge.Status.Unreachable -> {
                        stopPcDetect()
                        binding.pcDetectStatus.text = getString(R.string.pc_detect_status_fail)
                        Toast.makeText(requireContext(), R.string.pc_detect_status_fail, Toast.LENGTH_LONG).show()
                    }
                }
            }
        }
    ...
    private fun stopPcDetect() {
        pcDetectBridge.stop()
        if (_binding == null) return
        binding.pcDetectBtn.text = getString(R.string.pc_detect_start)
        binding.pcDetectStatus.text = getString(R.string.pc_detect_status_idle)
    }
